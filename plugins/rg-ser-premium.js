let handler = async (m, { conn, text }) => {
  ...
  if (!text) return conn.reply(m.chat, '*☁️ Ingresa el tipo de membresía y la duración.*\n> Ejemplo: #comprarpremium 1 dia', m);
  ...
  if (!units[unit.toLowerCase()]) return conn.reply(m.chat, '☁️ Unidad de tiempo no válida. Usa minutos, horas o días.', m);
