...
global.authFile = 'vegetaSession'
global.authFileJB  = 'vegetaJadiBot'
...
console.log(chalk.bgBlack(chalk.bold.greenBright(`🔥 POR FAVOR, INGRESE NUMERO DE WHATSAPP.\n${chalk.bold.yellowBright(`🔥 Ejemplo: 57321×××××××`)}\n${chalk.bold.magentaBright('---> ')}`)))
process.exit(0)
}} else {
while (true) {
numeroTelefono = await question(chalk.bgBlack(chalk.bold.greenBright(`🍬 POR FAVOR, ESCRIBA SU NUMERO DE WHATSAPP.\n 🔥  Ejemplo: 57321×××××××\n`)))
numeroTelefono = numeroTelefono.replace(/[^0-9]/g, '')

if (numeroTelefono.match(/^\d+$/) && Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
break 
} else {
console.log(chalk.bgBlack(chalk.bold.greenBright(`🔥 POR FAVOR, ESCRIBA SU NUMERO DE WHATSAPP.\n🔥  Ejemplo: 57321×××××××\n`)))
}}
rl.close()  
} 
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "vegetaJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
console.log('🌠 VEGETA-BOT-MB no tiene Sub-Bots vinculados.');
return;
}
...
console.log(chalk.bold.greenBright(`🌠 MB VEGETA conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar VEGETA-BOT: `, error))
}
})();
...
const directories = ['./vegetaSession/', './vegetaJadiBot/']
