...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${sessions}/creds.json`)) {
  do {
    ...
    if (!/^[1-2]$/.test(opcion)) console.log(chalk.bold.redBright(`✰ཽ No se permiten numeros que no sean 1 o 2, tampoco letras o símbolos especiales Saiyajin☁️.`))
  ...
  if (opcion === '2' || methodCode) {
    ...
    if (!conn.authState.creds.registered) {
      ...
      if (!!phoneNumber) addNumber = phoneNumber.replace(/[^0-9]/g, '')
        ...
        setTimeout(async () => {
          let pairingResponse = await conn.requestPairingCode(addNumber)
          let codeBot = typeof pairingResponse === 'string' ? pairingResponse : pairingResponse?.code || pairingResponse?.pin || ''
